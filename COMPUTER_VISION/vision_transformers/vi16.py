...
def train_model(model, train_loader, val_loader, device, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in train_loader:
            ...
            train_probs.extend(probs[:, 1].detach().cpu().numpy())  # Probabilidad de la clase maligna
...
def evaluate_model(model, test_loader, device):
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            ...
            test_probs.extend(probs[:, 1].detach().cpu().numpy())  # Probabilidad de la clase maligna
...
def visualize_gradcam(model, test_loader, device, num_images=5):
    # Importar después de asegurarnos de que esté instalado
    try:
        from pytorch_grad_cam import GradCAM
        from pytorch_grad_cam.utils.model_targets import ClassifierOutputTarget
        from pytorch_grad_cam.utils.image import show_cam_on_image
    except ImportError:
        print("pytorch-grad-cam no está instalado. Instálalo con: pip install pytorch-grad-cam")
        return
    
    model.eval()
    
    # Conseguir algunas imágenes del conjunto de prueba
    ...
    # Configurar GradCAM
    target_layers = [model.blocks[-1].norm1]  # Última capa de atención
    ...
    for i in range(num_images):
        ...
        # Convertir imagen a formato adecuado para visualización
        img_np = images[i].cpu().numpy().transpose(1, 2, 0)
        img_np = (img_np - img_np.min()) / (img_np.max() - img_np.min())  # Normalizar a [0,1]
        
        visualization = show_cam_on_image(img_np, grayscale_cam, use_rgb=True)
        
        # Mostrar imagen original y mapa de atención
        plt.subplot(num_images, 3, i*3+1)
        plt.imshow(img_np)
        plt.title(f'Original (True: {"Maligno" if labels[i] == 1 else "Benigno"})')
        plt.axis('off')
        ...
        plt.title(f'Pred: {"Maligno" if pred_class == 1 else "Benigno"} ({pred_prob:.2f})')
        plt.axis('off')
